    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: "#f8fafc", minHeight: "100vh", padding: "80px 48px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          <h2 style={{ fontSize: 48, fontWeight: 900, color: "#0D0D1A", letterSpacing: "-1.5px", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, alignItems: "start" }}>
          {plans.map(plan => (
            <div key={plan.name} style={{
              background: plan.highlight ? "#0D0D1A" : "#fff",
              border: plan.highlight ? "2px solid #1A3FF0" : "1.5px solid #e2e8f0",
              borderRadius: 20,
              padding: "36px 32px",
              position: "relative",
              boxShadow: plan.highlight ? "0 8px 40px rgba(26,63,240,0.25)" : "0 2px 12px rgba(0,0,0,0.06)",
              transform: plan.highlight ? "scale(1.03)" : "scale(1)",
            }}>
              {plan.badge && (
                <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg, #1A3FF0, #BB00FF)", color: "#fff", fontSize: 12, fontWeight: 700, padding: "4px 16px", borderRadius: 100, whiteSpace: "nowrap" }}>
                  {plan.badge}
                </div>
              )}
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: plan.highlight ? "#94a3b8" : "#64748b", textTransform: "uppercase", letterSpacing: 1 }}>{plan.name}</span>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 6 }}>
                <span style={{ fontSize: 52, fontWeight: 900, color: plan.highlight ? "#fff" : "#0D0D1A", letterSpacing: "-2px" }}>{plan.price}</span>
              ...
              <a style={{
                ...
                background: plan.highlight ? "linear-gradient(135deg, #1A3FF0, #BB00FF)" : "rgba(26,63,240,0.08)",
                color: plan.highlight ? "#fff" : "#1A3FF0",
              ...
              <div style={{ borderTop: `1px solid ${plan.highlight ? "rgba(255,255,255,0.08)" : "#f1f5f9"}`, paddingTop: 24 }}>
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
                  <li style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                    <span style={{ background: "linear-gradient(135deg, #1A3FF0, #BB00FF)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontWeight: 900, fontSize: 15, marginTop: 1 }}>✓</span>
        ...
        <div style={{ textAlign: "center", marginTop: 48, padding: "32px", background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0" }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 800, color: "#0D0D1A", marginTop: 8 }}>30-day money-back guarantee</div>
