...
class Ruleslegacy extends Db\Dataobjectsortedlist
{
	/**
	 ...
	 * @param System\Keystore $store         A wbLib keystore where ordering list will be stored.
	 */
	public function __construct($dataObjectClass = null, $orderingScope = '', $store = null)
	{
		...
		$this->orderingScope = empty($this->orderingScope)
			? 'rules'
			: $this->orderingScope;

		$this->orderingStore = empty($this->orderingStore)
			? $this->factory->getThe('forseo.keystore')
			: $this->orderingStore;
	...
	/**
	 * @onAfterInitialize Remove all rules that don't match the URL specification
	 */
	...
	public function filterByContentType()
	{
		...
		foreach (static::$rules as $ruleType => $rules)
		{
			static::$rules[$ruleType] = array_values(
				array_filter(
					static::$rules[$ruleType],
					function ($rule) use ($currentRequestExtension)
					{
						if ($rule->get('enabled') == Data\Rule::ENABLED)
						{
							return true;
						...
						if (empty($currentRequestExtension))
						{
							// on a 404 or other error page, extension may be empty
							return false;
						}
