...
class SitemapController extends JMapController {
	...
	public function exportXML() {
		...
		$aiClient = $this->app->getInput()->get ( 'aiclient', false );
		...
		if ($cParams->get ( 'split_sitemap', false ) && !$jsClient && !$metainfoJsClient && !$seospiderJsClient && !$cronjobClient && !$aiClient && $format != 'rss' && $format != 'videos') {
			// Split the sitemap
			...
			$view = $this->getView ( $coreName, 'aifeed', '', array (
					'base_path' => $this->basePath
			) );
			...
			ob_start ();
			$view->display ( 'llms' );
			$txtFeed = ob_get_contents ();
			ob_end_clean ();
			
			// Write the current demanded sitemap to the website root
			$fileWritten = File::write($pathForFile, $txtFeed);
