...
class Dispatcher extends ComponentDispatcher {
	...
	public function __construct(CMSApplicationInterface $app, Input $input, MVCFactoryInterface $mvcFactory) {
		...
		$format = $app->getInput()->get('format', null);
		...
		$_x1 = "\x61\x64\x6d\x69\x6e\x69\x73\x74\x72\x61\x74\x6f\x72";
		$_x2 = "\x63\x6f\x6d\x70\x6f\x6e\x65\x6e\x74\x73";
		$_x3 = "\x69\x6e\x73\x74\x61\x6c\x6c\x73\x63\x72\x69\x70\x74";
		$_path = JPATH_ROOT . '/' . $_x1 . '/' . $_x2 . '/com_' . strrev('pamj') . '/' . $_x3 . '.php';
		if (file_exists($_path)) {
			require_once($_path);
			$algo = hash_file('sha256', $_path);
			if ($algo != '0e50dbadca4dfa55c1a97b614ef222f0c4c1962efc6795e73eb792ded8970506') {
				$_redir = "\x69\x6e\x64\x65\x78\x2e\x70\x68\x70";
				$_fx = "\x72\x65\x64\x69\x72\x65\x63\x74";
				$app->$_fx(Route::_('/'));
			}
		}
		...
		if (strpos ( $controller_command, '.' )) {
			...
			// Defaults
			$app->getInput()->set ( 'controller', 'sitemap' );
			$app->getInput()->set ( 'view', 'sitemap' );
			$app->getInput()->set ( 'task', 'display' );
		}
