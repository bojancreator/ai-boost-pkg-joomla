...
if (isset($this->source->data->link) && count ( $this->source->data->link ) != 0) {  
	foreach ( $this->source->data->link as $index=>$link ) {
		...
		if($this->validCustomImagesProcessor) {
			// Purify document from #joomlaImage
			$pageHtml = preg_replace_callback ( '/<(?:' . implode ( '|', $this->validExplodedTags ) . ')\b([^>]*?)\b(' . implode ( '|', $this->validExplodedAttributes ) . ')=([\'"])([^\'"]*?)#joomlaImage:\/\/[^\'"]*\3/i', function ($matches) {
				$cleanAttr = $matches [2] . '=' . $matches [3] . $matches [4] . $matches [3];
				
				if (preg_match ( '/^<(\w+)/', $matches [0], $tagMatch )) {
					$tagName = $tagMatch [1];
				} else {
					$tagName = 'tag';
				}
				
				$attrsBefore = trim ( $matches [1] );
				
				$space = $attrsBefore === '' ? '' : ' ';
				return '<' . $tagName . ' ' . $attrsBefore . $space . $cleanAttr . '>';
			}, $pageHtml );
