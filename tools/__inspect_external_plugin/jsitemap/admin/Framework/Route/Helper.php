...
final class Helper {
	...
	protected static function findItem($option, $needles, $manifestObject, $thisView) {
		...
		if (! isset ( $lookup [$option] )) {
			...
			foreach ( $items as $item ) {
				...
				// Special management overwriting id zero with another custom query string param
				if(isset($manifestObject['forced_property_view_needle']) && $item ['query'] ['view'] == $manifestObject['forced_property_view_needle'] && $item ['query'] ['id'] == 0) {
					$item ['query'] ['id'] = $item ['query'] [$manifestObject['forced_property_view_param']];
					$lookup [$option] [$item ['query'] ['view']] [0] = $item ['id'];
				}
				...
				if (isset ( $item ['query'] ) && isset ( $item ['query'] ['view'] )) {
					...
					} elseif (isset ( $item ['query'] ['cid'] ) && !is_array($item ['query'] ['cid'])) { // CID identifier for categories
						// No override already more specific itemid for a certain language
					...
					} elseif (isset ( $item ['query'] ['cid'] ) && is_array($item ['query'] ['cid'])) { // CIDs array identifier for categories
						// No override already more specific itemid for a certain language
						foreach ($item ['query'] ['cid'] as $singleCid) {
							if (isset ( $lookup [$option] [$view] [$singleCid] )) {
								$alreadyAssigned = $lookup [$option] [$view] [$singleCid];
								$alreadyAssignedLanguage = $items [$alreadyAssigned] ['language'];
								if ($alreadyAssignedLanguage != '*' && $item ['language'] == '*') {
									continue 2;
								}
							...
							// Standard assignment using catid query identifier
							$lookup [$option] [$view] [$singleCid] = $item ['id'];
						}
	...
	public static function getItemRoute($option, $thisView, $id, $elm, $mainTable) {
		...
		// Add if any additional custom components entity views
		if(isset($manifests [$option] ['forced_property_view_needle'])) {
			$needles [$manifests [$option] ['forced_property_view_needle']] = array (
				( int ) $elm->{$manifests [$option] ['forced_property_view_needle']}
			);
		}
