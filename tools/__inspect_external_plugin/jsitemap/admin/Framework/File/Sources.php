...
class Sources extends BaseError {
	...
	public function import() {
		...
		try {
			...
			foreach ($objectToRestore as $dataSource) {
				...
				if($dataSource->type == 'plugin' && !is_dir(JPATH_COMPONENT_ADMINISTRATOR . '/plugins/' . strtolower($dataSource->name))) {
	...
	public function install() {
		// Get file info
		$file = $this->app->getInput()->files->get('datasourceinstallplugin', null, 'raw');
		$tmpFile = $file['tmp_name'];
		$tmpFileName = $file['name'];
		try {
			if(!$tmpFile || !$tmpFileName) {
			...
			if($folderPluginName != strtolower($pluginName)) {
				...
				if(is_dir(JPATH_COMPONENT_ADMINISTRATOR . '/plugins/' . strtolower($pluginName))) {
