...
class AigeneratorController extends JMapController {
	...
	public function saveEntity(): bool {
		...
		if(!$result = $model->storeEntity()) {
			...
			$this->setRedirect ( 'index.php?option=com_jmap&task=aigenerator.editEntity&cid[]=' . $this->app->getInput()->get ( 'id' ), Text::_('COM_JMAP_ERROR_SAVING'));
			return false;
		}
	...
	public function generateEntity(): bool {
		...
		$keywordsPhrase = $this->app->getInput()->getString('keywords_phrase');
		$selectedApi = $this->app->getInput()->get('api');
		$model->setState('maxresults', $this->app->getInput()->getInt('maxresults', $model->getComponentParams('aigenerator_default_results', 10)));
		$model->setState('temperature', $this->app->getInput()->getString('temperature', '0.5'));
		...
		$model->setState('contentlanguage', $this->app->getInput()->getString('language', $defaultLanguageCode));
		
		$model->setState('removeimgs', $this->app->getInput()->getInt('removeimgs', 0));
		...
		switch ($selectedApi) {
			...
			case 'openai':
				$generatedAIContentsResult = $model->generateOpenAIContentData($keywordsPhrase);
				break;
		}
		...
		if($generatedAIContentsResult === false) {
			...
			// Check if it was a previous Openai record or if the keywords changes, then delete images folder
			if($idRecord = $this->input->get('id')) {
				$currentRecord = $model->loadEntity($idRecord);
				if (($selectedApi != $currentRecord->api && $currentRecord->api == 'openai') || ($keywordsPhrase != $currentRecord->keywords_phrase && $selectedApi == 'openai')) {
					// If AI model is ChatGPT, delete also local images files
					$imagesFolder = md5 ( $currentRecord->keywords_phrase );
