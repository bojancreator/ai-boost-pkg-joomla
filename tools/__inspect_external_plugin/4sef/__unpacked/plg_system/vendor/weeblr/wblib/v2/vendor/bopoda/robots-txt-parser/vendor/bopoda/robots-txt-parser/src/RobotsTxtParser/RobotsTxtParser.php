...
namespace Weeblr\Wblib\Forsef\RobotsTxtParser;
...
class RobotsTxtParser
{
	...
	{
		// Weeblr: don't die if mbstring does not exists
	...
	public function getRules($userAgent = null)
	{
		...
		else
		{
			...
			$userAgent = StringHelper::strtolower($userAgent);
			// Weeblr
	...
	private function prepareRules()
	{
		...
		$endOfLine = PHP_EOL;
		if (strpos($this->content, PHP_EOL) === false && strpos($this->content, "\n") !== false)
		{
			$endOfLine = "\n";
		}
		else if (strpos($this->content, PHP_EOL) === false && strpos($this->content, "\r") !== false)
		{
			$endOfLine = "\r";
		}

		$rows = explode($endOfLine, $this->content);
		// weeblr
