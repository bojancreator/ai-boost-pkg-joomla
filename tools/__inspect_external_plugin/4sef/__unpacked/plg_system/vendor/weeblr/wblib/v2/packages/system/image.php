...
class Image
{
	...
	public function getHandle()
	{
		...
		if (!$this->isLoaded())
		{
			throw new \Exception('No valid image was loaded.');
		}
	...
	public static function getImageFileProperties($path)
	{
		...
		if (!$info)
		{
			throw new \Exception('Unable to get properties for the image.');
		}
	...
	public function getOrientation()
	{
		if ($this->isLoaded())
		{
			return self::getOrientationString(
				$this->getWidth(),
				$this->getHeight()
			);
		}
	...
	/**
	 * Weeblr: fast rescale. Use instead of resize() if entire image should be resized.
	 * Original resize does computation on width and height and this leads to rounding
	 * errors by one pixel on the dimensions.
	 *
	 * @param   int  $width
	 * @param   int  $height  -1 to keep aspect ratio
	 * @param   int  $mode
	 *
	 * @return Image
	 */
	public function safeResize($width, $height = -1, $mode = IMG_BILINEAR_FIXED)
	{
		$handle = imagescale($this->getHandle(), $width, $height, $mode);
	...
	public function flip($mode, $createNew = true)
	{
		...
		{
			throw new \Exception('Unable to flip the image.');
		}
