...
{
	return ($langName == Factory::get()->getThe('platform')->getDefaultLanguageTag() || $langName == '*');
}
...
// wbremove, likely notneeded any more
...
function shGetDefaultDisplayNum($menuItemid, $url, $fromSession = false, $includeBlogLinks = false)
{
	...
	if (!empty($menuItemid))
	{
		...
		{
			$compParams = clone JComponentHelper::getParams($currentOption);
					...
					if ($urlType == sh404SEF_URLTYPE_NONE)
					{
						...
						)
						{
							...
							else
							{
								...
								//$shSefString = shAdjustToRewriteMode($sefstring); // this function does not do anything actually
						...
						else
						{
							...
							/**
							 * Filter whether to bypass the SEF URL creation just before it happens.
							 *
							 * @api
							 * @package sh404SEF\filter\urls
							 * @var sh404sef_leave_non_sef
							 * @since   4.23.0
							 *
							 * @param boolean $shouldByPass If true, the sef variable will not be created.
							 * @param string  $nonSef       The full non-sef URL to convert
							 * @param array   $vars         The input variables for this URL.
							 *
							 * @return boolean
							 */
							$bypassSef = ShlHook::filter(
								'sh404sef_leave_non_sef',
								false,
								$string,
								$URI->querystring
							);
							...
							{
								$sefstring = $string;
								ShlSystem_Log::debug('sh404sef', 'Filter said to bypass sef url creation: ' . $sefstring);
							}
							else
							{
...
function getMenuTitle($option, $task, $id = null, $string = null, $shLanguage = null)
{
	...
	$menuItem = shFindMenuItem($menu->getMenu('site'), $attr, $values, $firstOnly = true);
	...
	if (!empty($menuItem))
	{
		...
		{
			if (!empty($pageInfo->homeLinks[$language->lang_code]) && strpos($pageInfo->homeLinks[$language->lang_code], 'Itemid=' . $menuItem->id) !== false)
			{
				...
				return $title; // this is one of the homepages, return / or a lang code
			}
		}
		// non-homepage
...
function shGetArticleIdString($id, $view, $option, $shLangName)
{
	...
	if ($sefConfig->ContentTitleInsertArticleId && isset($sefConfig->shInsertContentArticleIdCatList) && !empty($id) && ($view == 'article'))
	{
		 ...
		 * @return array
		 */
		$shouldInserArticleId = ShlHook::filter(
			'sh404sef_should_insert_article_id_in_title',
			true,
			'page_title',
			$article[$shLangName]
		);
		if (!$shouldInserArticleId)
		{
			return $articleId;
		}
