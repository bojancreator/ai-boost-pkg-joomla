...
class Sh404sefClassConfig extends Base\Base
{
...
//		$values = Sh404sefHelperGeneral::getComponentParams($reset)->toArray();
		...
		$values = Factory::get()->getThis('forsef.config', 'legacy')
						 ->toArray();
...
//		$app = JFactory::getApplication();
		...
		if (!empty($pageInfo->isMultilingual) && $this->platform->isFrontend())
		{
			$ret = $currentList;
		...
		else
		{
			...
			else
			{
				foreach ($activeLanguages as $language)
				{
					...
					{
						$ret[$language->lang_code] = $language->lang_code == $pageInfo->currentLanguageTag ? $defaultLangDefault : $default;
					}
					else
					{
						$ret[$language->lang_code] = $currentList[$language->lang_code];
					}
				}
			}
		}
