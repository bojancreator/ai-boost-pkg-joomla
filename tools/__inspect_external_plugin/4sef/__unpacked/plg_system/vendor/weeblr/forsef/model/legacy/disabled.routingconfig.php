...
namespace Weeblr\Forsef\Model\Legacy;
...
class Routingconfig extends Base\Base
{
	...
	public $version = '@ant_version_ant@';
	...
	public $shAlwaysInsertItemid = false;
	...
	public $guessItemidOnHomepage = false;
	...
	public $shCBShortUserURL = false;
	// a set of boolean vars, to decide what to do with existing data when upgrading sh404SEF
	public $shKeepStandardURLOnUpgrade     = true;
	public $shKeepCustomURLOnUpgrade       = true;
	public $shKeepMetaDataOnUpgrade        = true;
	public $shKeepModulesSettingsOnUpgrade = true;
	public $shMultipagesTitle              = true;
...
//	public $shAdminInterfaceType = SH404SEF_STANDARD_ADMIN;
	...
	// V 2.1.10
	public $useJoomsefRouter = [];
	public $useAcesefRouter  = [];
	...
	public $useJoomlaRouterPhpWithItemid = [];
	...
	public $itemidOverridesIfMissing          = [];
	public $itemidOverridesAlways             = [];
	public $itemidOverridesValues             = [];
...
//		$values = Sh404sefHelperGeneral::getComponentParams($reset)->toArray();
		...
		if (!empty($values))
		{
			...
			$this->nocache               = [];
			$this->skip                  = [];
			$this->useJoomlaRouter       = [];
			$this->notTranslateURLList   = [];
			$this->notInsertIsoCodeList  = [];
			$this->shDoNotOverrideOwnSef = [];
//			$this->useJoomlaRouterPhpWithItemid = [];
//			$this->useJoomsefRouter             = [];
//			$this->useAcesefRouter              = [];
			$this->compEnablePageId = [];
//			$this->shLangTranslateList          = [];
//			$this->shLangInsertCodeList         = [];
			$this->defaultComponentStringList = [];
			$this->itemidOverridesIfMissing   = [];
			$this->itemidOverridesAlways      = [];
		...
		/*	else
			{
				...
				if (shFileExists($sefConfigFile))
				{
					...
					$this->liveSites = $this->shInitLanguageList(isset($liveSites) ? $liveSites : [], '', '');
...
//		unset($shDefaultParamsHelp);
		...
		// compatibility variables, for sef_ext files usage from OpenSef/SEf Advance V 1.2.4.p
...
//		$joomlaUrlRewriting = (bool)($j3 ? $app->get('sef_rewrite') : $app->getCfg('sef_rewrite'));
		$this->shRewriteMode = $this->platform->getConfig()->get('sef_rewrite') ? 0 : 1;
	}

	protected function initLanguageList($currentList, $default, $defaultLangDefault)
	{
		$list               = [];
		...
		if (
			...
			&&
			!$this->platform->isMultilingual()
		)
		{
			$list = $currentList;
			if (empty($currentList) || !isset($currentList[$currentLanguageTag]))
			{
				$list[$currentLanguageTag] = $defaultLangDefault;
			}

			return $list;
		}

		$activeLanguages = $this->platform->getFrontendLanguages();
		if (empty($activeLanguages))
		{
			...
			{
				$list[$currentLanguageTag] = $currentList[$currentLanguageTag];
			}
		}
		else
		{
			...
			{
				if (empty($currentList) || !isset($currentList[$language->lang_code]))
				{
					$list[$language->lang_code] = $language->lang_code == $currentLanguageTag
						? $defaultLangDefault
						:
						$default;
				}
				...
				{
					$list[$language->lang_code] = $currentList[$language->lang_code];
				}
			}
		}
	...
	public function getReplacementsList()
	{
		...
		foreach ($items as $item)
		{
			if (!empty($item))
			{
				...
				$replacementsList[StringHelper::trim($src)] = StringHelper::trim($dst);
			}
		}
	...
	public function getStripCharList()
	{
